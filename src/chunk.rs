    ...
    pub fn chunks_by_token_count_bge_small_en_v15(document: &str, max_tokens: i32, max_overlap: i32) -> Vec<&str> {
        ...
        CELL.with(|cell| {
            let (tokenizer, model_max_length) = cell.get_or_init(|| {
                let mut tokenizer = Tokenizer::from_bytes(include_bytes!("../bge_small_en_v15/tokenizer.json"))
                    .expect_or_pg_err("Error loading tokenizer");

                let special_tokens_map: serde_json::Value =
                    serde_json::from_slice(include_bytes!("../bge_small_en_v15/special_tokens_map.json"))
                ...
                let tokenizer_config: serde_json::Value =
                    serde_json::from_slice(include_bytes!("../bge_small_en_v15/tokenizer_config.json"))
